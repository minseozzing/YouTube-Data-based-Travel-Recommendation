    ...
    public OAuth2User loadUser(OAuth2UserRequest userRequest) throws OAuth2AuthenticationException {
        ...
        if (member == null) {
            member = Member.builder()
                    ...
                    .role("ROLE_USER")
                    .build();
            log.info("[CustomOAuth2UserService] ?†Í∑ú ?åÏõê ?ùÏÑ±: socialId={}", socialId);
        } else {
            // Î°úÍ∑∏???åÎßà??ÏµúÏã† ?ÑÎ°ú?ÑÎ°ú ?ôÍ∏∞??
            member.updateProfile(nickname, profileImageUrl);
            log.info("[CustomOAuth2UserService] Í∏∞Ï°¥ ?åÏõê ?ÑÎ°ú??Í∞±Ïã†: socialId={}", socialId);
        }

        // Íµ¨Í? ?†ÌÅ∞ ?ÖÎç∞?¥Ìä∏ (refresh_token?Ä SuccessHandler??Î≥ÑÎèÑ Ï≤òÎ¶¨Í∞Ä ?ÑÏöî?????àÏùå)
